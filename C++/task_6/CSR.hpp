...
class CSRMatrix : public SparseMatrixAbstract<DataType, OffsetType, IndexType>
{
    private:
...
CSRMatrix<DataType, OffsetType, IndexType>::CSRMatrix(IndexType m, IndexType n, std::istream &is) : m_offsets(read_offsets(m, is)),
...
template <class DataType, class OffsetType, class IndexType>
void CSRMatrix<DataType, OffsetType, IndexType>::matvec(const DataType *x, DataType *y, bool Transpose) const
{
    for (IndexType i = 0; i < row_number(); i++)
    {
        auto start = m_elements.begin() + m_offsets[i];
